# Using LIWC data for the same books but with download data for 2018-07-22

##Hypotheses

# H0: There's no difference in the LIWC tests which produce significant results between the 2014 and 2018 data
# HA: There is a difference in the LIWC tests which produce significant results between the 2014 and 2018 data
...
#load LIWC data
lf<- read_excel("readability_data.xlsx",sheet = 7) #load LIWC 2018 data
names(lf)<- tolower(names(lf))


mf<- read_excel("readability_data.xlsx",sheet = 12) #load LIWC absolute data
...
lf<- lf %>% rename(dicWo=`dictionarywords`,sixletter=`6letter`,mWoSen=`words-per-sentence`,functional=`function`, ranking=`ranking_2018`)

mf<- mf %>% rename(dicWo=`dictionarywords`,sixletter=`6letter`,mWoSen=`words-per-sentence`,functional=`function`, ranking=`ranking_2018`)
...
write.csv(t1,"t1_LIWC_2018.csv") #write to excel
...
ggsave("LIWC categories - 2018 data.png", 
       plot = last_plot(), 
...
write.csv(x$importance,"x_LIWC_2018.csv")
...
write.csv(p.df,"LIWC_tag_difference_2018.csv")
...
tt <- t.df  %>% mutate(status=t.df$rank,genre=t.df$genre) %>% select(genre, status, functional:otherp)
...
write.csv(tt4,"LIWC_tag_difference_new_2018.csv")
...
ggsave("LIWC Difference in proportion function-article - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion prep-number - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion quant-friend - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion female-percept - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion see-affiliation - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion achieve-space - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion time-netspeak - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion assent-exclam - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion dash-otherp - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
write.csv(fx,"clout_fx_LIWC_2018.csv")
...
ggsave("LIWC - Clout - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(analytic),s=sd(analytic)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"analytic_fx_LIWC_2018.csv")
...
ggsave("LIWC - Analytic - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(authentic),s=sd(authentic)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"authentic_fx_LIWC_2018.csv")
...
ggsave("LIWC - Authentic - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(tone),s=sd(tone)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"tone_fx_LIWC_2018.csv")
...
ggsave("LIWC - Tone - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(sixletter),s=sd(sixletter)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"sixletter_fx_LIWC_2018.csv")
...
ggsave("LIWC - Six letter words - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(dicWo),s=sd(dicWo)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"dicWo_fx_LIWC_2018.csv")
...
ggsave("LIWC - Dictionary words - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(mWoSen),s=sd(mWoSen)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"mWoSen_fx_LIWC_2018.csv")
...
ggsave("LIWC - Mean words per sentence - by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
