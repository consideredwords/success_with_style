# Using LIWC data for the same books but with only the first 3,000 words for the books from 2013-10-24

##Hypotheses

# H0: There's no difference in the LIWC tests which produce significant results between the full book data and for the first 3,000 words
# HA: There is a difference in the LIWC tests which produce significant results between the full book data and for the first 3,000 words
...
#load LIWC data
lf<- read_excel("readability_data.xlsx",sheet = 10) #load LIWC 3,000 words data
names(lf)<- tolower(names(lf))


mf<- read_excel("readability_data.xlsx",sheet = 11) #load LIWC absolute data
...
write.csv(t1,"t1_LIWC_3k.csv") #write to excel
...
ggsave("LIWC categories - 3k words.png", 
       plot = last_plot(), 
...
write.csv(x$importance,"x_LIWC_3k.csv")
...
write.csv(p.df,"LIWC_tag_difference_3k.csv")
...
tt <- t.df  %>% mutate(status=mf$ranking,genre=mf$genre) %>% select(genre, status, functional:otherp)
...
write.csv(tt4,"LIWC_tag_difference_new_3k.csv")
...
ggsave("LIWC Difference in proportion function-article - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion prep-number - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion quant-friend - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion female-percept - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion see-affiliation - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion achieve-space - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion time-netspeak - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion assent-exclam - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("LIWC Difference in proportion dash-otherp - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
write.csv(fx,"clout_fx_LIWC_3k.csv")
...
ggsave("LIWC - Clout - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(analytic),s=sd(analytic)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"analytic_fx_LIWC_3k.csv")
...
ggsave("LIWC - Analytic - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(authentic),s=sd(authentic)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"authentic_fx_LIWC_3k.csv")
...
ggsave("LIWC - Authentic - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(tone),s=sd(tone)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"tone_fx_LIWC_3k.csv")
...
ggsave("LIWC - Tone - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(sixletter),s=sd(sixletter)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"sixletter_fx_LIWC_3k.csv")
...
ggsave("LIWC - Six letter words - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(dicWo),s=sd(dicWo)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"dicWo_fx_LIWC_3k.csv")
...
ggsave("LIWC - Dictionary words - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
fx<- dx %>% group_by(genre,rank) %>% summarise(m=mean(mWoSen),s=sd(mWoSen)) %>% gather(key,value,c(m,s)) %>% 
  dcast(genre~rank+key,value.var = "value")
write.csv(fx,"mWoSen_fx_LIWC_3k.csv")
...
ggsave("LIWC - Mean words per sentence - by genre - 3k words.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
