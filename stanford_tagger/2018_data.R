##Recreation of the original 'Success with style' experiment using data (data downloaded 2018-07-22) as compared with 2013-10-24 data
...
df<- read_excel("readability_data.xlsx",sheet = 6) #load 2018 proportional data
names(df)<- tolower(names(df))
df<- df %>% rename(FR=`fleschkincaid readability`,mSyWo=`mean-syllables-per-word`, mWoSen=`mean-words-per-sentence`, ranking=`ranking_2018`)


#load absolute (original) data
ef<- read_excel("readability_data.xlsx",sheet = 5) #load 2018 absolute original data
names(ef)<- tolower(names(ef))
ef<- ef %>% rename(ranking=`ranking_2018`)
...
write.csv(t1,"t1_2018.csv") #write to excel
...
ggsave("Readability, mean words per sentence and mean syllables per word - 2018 data.png", 
       plot = last_plot(), 
...
write.csv(x$importance,"x_2018.csv")
...
write.csv(p.df,"original_tag_output_2018.csv")
...
gg4 <- gg3 %>% mutate(rowSum = rowSums(.[,names(gg3)[3:42]])) %>% 
  group_by(genre) %>% 
  summarise_at(vars(names(gg3)[3:42]),   
               funs(net = .[status == "SUCCESS"]/rowSum[status == "SUCCESS"] - 
                      .[status == "FAILURE"]/rowSum[status == "FAILURE"] )) %>%
  as.data.frame()

write.csv(gg4,"gg4_2018.csv")
...
ggsave("Difference in proportion (original tags) rb-vbz - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("Difference in proportion (original tags) wp-prp - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("Difference in proportion (all tags) cc-ls - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("Difference in proportion (all tags) md-rbs - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("Difference in proportion (all tags) sym-wdt - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
ggsave("Difference in proportion (all tags) wp-wrb - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
...
write.csv(ff8,"ff8_2018.csv")

#absolute - all tags

gg6 <- setDT(gg)[,lapply(.SD,sum),status,.SDcols=3:42][,
                                                       .SD/rowSums(.SD),.SDcols=-1][,rbind(.SD,.SD[1]-.SD[2])]

gg7 <- gg6[-c(1,2),]

gg8<- gg7 %>% gather(POS,diff)  %>% arrange(desc(diff))
write.csv(gg8,"gg8_2018.csv")
...
ggsave("Readability by genre - 2018 data.png", 
       plot = last_plot(), 
       width = 260, height = 171, 
       units = "mm", # other options c("in", "cm", "mm"), 
       dpi = 240)
